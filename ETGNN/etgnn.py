    ...
    if args.mode == 0:
        ...
        for e in range(epoch):
            ...
            if args.use_uncertainty:
                ...
                label_u = torch.FloatTensor(train_i_u)
...
def EUC_loss(alpha, u, true_labels, e):
    ...
    a = -0.01 * torch.exp(-(e + 1) / 10 * torch.log(torch.FloatTensor([0.01])))
...
if __name__ == "__main__":
    ...
    # dataset = Event2012()  # dataframe
    project_path = os.getcwd()
    load_path = os.path.dirname(project_path) + "/dataset/raw dataset"
    ...
    df = df[['tweet_id', 'text', 'event_id', 'words', 'filtered_words',
    ...
    dataset = df[(df['date'] >= init_day + datetime.timedelta(days=0)) & (
